...
- Blocking issues: 0
- Reconciliation tensions: 0
- Mutation/deletion failures: 0

## Next action

- No blocking structural diagnostics. A conservative rewrite can be prepared if desired.

## Diagnostics

- Fact schema warning: [fact-warning w-long-text long-text predicate comment slot 2 length 314]
- Fact schema warning: [fact-warning w-long-text long-text predicate definition slot 2 length 295]
- Fact schema warning: [fact-warning w-suspicious suspicious-atom predicate term atom entity reason generic-placeholder]
- Fact schema warning: [fact-warning w-suspicious suspicious-atom predicate term atom entity reason generic-placeholder]
- Fact schema warning: [fact-warning w-long-text long-text predicate definition slot 2 length 293]
...
- c1 (main-claim, produces claim): Predictive policing increases social trust. EXTRACTED from opening sentence.
- c2 (core-condition, causal claim): Visible order reduces uncertainty. EXTRACTED: 'visible order reduces uncertainty.'
- c3 (core-condition, causal claim): Uncertainty makes strangers feel dangerous. EXTRACTED: 'uncertainty is what makes strangers feel dangerous.'
- c4 (core-condition, causal claim): Communities with fewer perceived threats cooperate more easily. EXTRACTED from the text.
- g1 (ground claim): Visible order directly reduces uncertainty about the social environment. EXTRACTED from the causal chain in the text.
- g2 (ground claim): Surveillance cameras function as civic reassurance — surveillance increases visibility. EXTRACTED: 'surveillance cameras function as civic reassurance' and 'transparency and surveillance are adjacent concepts.'
- g3 (ground claim): Visibility is the precondition for social confidence. EXTRACTED from the closing sentence.
- uncertaintyreductionchain: Visible order (visibleorder) reduces uncertainty (uncertaintyreduction). Reduced uncertainty lowers danger perception. Lower danger perception enables cooperation. Cooperation increases social trust (socialtrust). EXTRACTED from the text's causal chain.
- surveillancestrvisibility: Surveillance (surveillance) functions as visibility (visibility) — cameras make behavior observable. Observable behavior is predictable. Predictable behavior enables social confidence (socialconfidence). EXTRACTED: 'transparency and surveillance are adjacent concepts: both increase visibility.'
- regulationstabilizesbehavior: Systems that observe behavior stabilize behavior — thermostats regulate temperature, classrooms regulate attention, prisons regulate violence. All stable societies regulate conduct. EXTRACTED from the text's analogical argument.
- o1 (core-condition, entity): Surveillance can produce fear, conformity, and selective enforcement. EXTRACTED: 'Critics argue that surveillance can also produce fear, conformity, and selective enforcement.'
- r1 (safeguard, reason): Seatbelts also restrict freedom, and nobody calls roads authoritarian. EXTRACTED from the text.
- o2 (core-condition, entity): This conflates coordination with coercion. EXTRACTED: 'Opponents say this conflates coordination with coercion.'
- r2 (safeguard, reason): Completely unregulated environments collapse into noise. Noise prevents trust because people cannot predict one another reliably. Therefore transparency and surveillance are adjacent — both increase visibility, and visibility is the precondition for social confidence. EXTRACTED from the text.

## File map

- Read first: output/check-report.md
- Human draft: work/draft.txt
- AI translation: work/ai-facts.shen
- Temporary per-run logic: work/adapter-facts.shen
- Expert raw flags: output/shen-output.txt
- Rewrite path, only when needed: work/rewrite-patch.json -> work/rewrite.md -> output/rewrite-report.md

## Expert appendix: raw Shen flags

- [fact-warning w-long-text long-text predicate comment slot 2 length 314]
- [fact-warning w-long-text long-text predicate definition slot 2 length 295]
- [fact-warning w-suspicious suspicious-atom predicate term atom entity reason generic-placeholder]
- [fact-warning w-suspicious suspicious-atom predicate term atom entity reason generic-placeholder]
- [fact-warning w-long-text long-text predicate definition slot 2 length 293]
- [plan-status p1 ready-for-final-rewrite]

## Expert appendix: source facts
...
## Expert appendix: user-supplied facts from gap answers

- none

## Expert appendix: temporary adapter facts
